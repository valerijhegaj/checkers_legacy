...
func TestField_Put(t *testing.T) {
	field := getTestField()
	for i := 0; i < 3; i++ {
		field.Put(Coordinate{i, i}, test_figure{1})
...
func TestField_At(t *testing.T) {
	...
	field.Put(Coordinate{1, 1}, test_figure{1})
	field.Put(Coordinate{2, 2}, test_figure{0})
...
func TestField_IsAvailable(t *testing.T) {
	...
	field.Put(Coordinate{1, 1}, test_figure{1})
	field.Put(Coordinate{2, 2}, test_figure{1})
...
func TestField_Move(t *testing.T) {
	field := getTestField()
	field.Put(Coordinate{0, 0}, test_figure{0})
	...
	if field.At(Coordinate{1, 1}) == nil {
		...
		t.Error()
	} else if reflect.TypeOf(field.At(Coordinate{1, 1})) != reflect.TypeOf(test_figure{}) {
		t.Error()
	}
...
func TestField_GetCopy(t *testing.T) {
	...
	copy := field.GetCopy()
	field.Remove(Coordinate{0, 0})
	field.Put(Coordinate{0, 1}, test_figure{1})
