...
func TestKing_IsMoveOne(t *testing.T) {
	...
	test := func(to Coordinate, returnValue bool) {
		if king.IsMoveOne(&field, from, to) != returnValue {
			t.Error()
		}
	}
...
func TestKing_Move_WithFood(t *testing.T) {
	...
	for x := 1; x < 8; x += 2 {
		for y := 1; y < 8; y += 2 {
			grandField.Put(Coordinate{x, y}, test_figure{1})
		}
	}
	...
	field := getTestField()
	field.Put(Coordinate{3, 3}, test_figure{1})
	...
	field = getTestField()
	field.Put(Coordinate{3, 3}, test_figure{0})
	field.Put(Coordinate{5, 3}, test_figure{0})
